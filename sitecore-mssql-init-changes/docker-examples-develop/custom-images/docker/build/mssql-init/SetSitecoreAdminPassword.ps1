...
if($SqlAdminUser -and $SqlAdminPassword) {
    & sqlcmd -I -i (Join-Path $ResourcesDirectory 'SetSitecoreAdminPassword.sql') -d 'DEV.Sitecore.Core' -S "$SqlServer" -U "$SqlAdminUser" -P "$SqlAdminPassword"
} else {
    & sqlcmd -I -i (Join-Path $ResourcesDirectory 'SetSitecoreAdminPassword.sql') -d 'DEV.Sitecore.Core' -S "$SqlServer"
}
